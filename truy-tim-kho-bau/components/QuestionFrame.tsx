...
const QuestionFrame: React.FC<QuestionFrameProps> = ({ question, onAnswer, onNext, isCorrect, isLastStage }) => {
  ...
  return (
    <div className="w-[85%] h-full mx-auto flex gap-6 bg-white/70 backdrop-blur-md rounded-3xl border-[6px] border-[#FF9800] p-6 shadow-2xl relative overflow-hidden">
      {/* Left: Stage Image */}
      <div className="w-1/3 h-full rounded-2xl overflow-hidden shadow-inner border-2 border-orange-200 bg-gradient-to-br from-orange-50 to-amber-50 flex items-center justify-center">
        {currentImage ? (
          <img
            src={currentImage}
          ...
          <div className="flex flex-col items-center gap-4 text-center p-6">
            <span className="text-8xl">{question.stageIcon}</span>
            <span className="text-2xl font-black text-orange-600">{question.stageName}</span>
          </div>
        )}
      </div>
