...
interface GameTuongTacProps {
    onBack: () => void;
}

const GameTuongTac: React.FC<GameTuongTacProps> = ({ onBack }) => {
    const [copySuccess, setCopySuccess] = useState(false);
    const [isShortening, setIsShortening] = useState(false);

    const IFRAME_PATH = '/gametuongtac/index.html';

    const handleDownloadZip = () => {
        const a = document.createElement('a');
        a.href = '/gametuongtac.zip';
        a.download = 'game-tuong-tac.zip';
        ...
        <div className="relative w-full h-screen bg-slate-900 flex flex-col">
            ...
            <div className="flex items-center gap-3 p-4 bg-slate-800/80 border-b border-slate-700">
                ...
                <div className="flex-1 min-w-0">
                    <h1 className="text-lg font-bold text-white">🎮 Game Tương Tác</h1>
                    <p className="text-xs text-slate-400">Học liệu tương tác dạng game</p>
                </div>
