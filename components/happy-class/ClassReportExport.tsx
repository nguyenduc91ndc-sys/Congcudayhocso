  ...
  const buildPrintHtml = () => {
    ...
    return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><title>${escapeHtml(reportTitle)}</title><style>
      ...
      *{box-sizing:border-box} body{margin:0;color:#1f2937;font:11px Arial,'Segoe UI',sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      ...
      table{width:100%;border-collapse:collapse;page-break-inside:auto} tr{page-break-inside:avoid} th,td{padding:5px 4px;border:1px solid #bcb2c3} th{color:#fff;background:#6d3296;font-size:8px;text-transform:uppercase} td{font-size:9px}.center{text-align:center}.number{text-align:right;font-weight:700}.empty{text-align:center;color:#777;padding:18px}
        ...
        <div className="class-report-body">
          ...
          <div className="class-report-preview-wrap">
            ...
            <div className={`class-report-paper ${orientation}`}>
              ...
              <div className="report-paper-table"><div className={`report-paper-table-head ${includeAttendance ? 'has-attendance' : ''}`}><span>HẠNG</span><span>HỌ VÀ TÊN</span><span>TỔ</span><span>ĐIỂM KỲ</span><span>TÍCH LŨY</span>{includeAttendance && <span>CHUYÊN CẦN</span>}</div>{report.rows.slice(0, 16).map((row) => <div className={`report-paper-row ${includeAttendance ? 'has-attendance' : ''}`} key={row.id}><span>{row.rank}</span><strong>{row.name}</strong><span>{row.team}</span><b>{signedPoints(row.points)}</b><span>{row.cumulativeScore}</span>{includeAttendance && <small>{row.present} mặt · {row.absent} vắng</small>}</div>)}{!report.rows.length && <p className="report-paper-empty">Chưa có dữ liệu trong kỳ báo cáo này.</p>}{report.rows.length > 16 && <p className="report-paper-more">… và {report.rows.length - 16} học sinh ở các trang tiếp theo</p>}</div>
