...
import { saveSharedThuMoi, updateSharedThuMoi, getSharedThuMoi, checkStudentRSVP, saveStudentRSVP, getUserThuMoiList, getRSVPs, deleteStudentRSVP } from '../utils/firebaseThuMoiDauNam';
...
const ThuMoiDauNam: React.FC<Props> = ({ onBack, sharedId, user, onRequireLogin }) => {
  ...
  const loadUserList = async () => {
    ...
    try {
      ...
      console.error('[ThuMoiDauNam] Cannot load user invitations:', error);
      ...
      if (sharedId) {
        ...
        try {
          ...
          if (config) {
            ...
            setIframeSrc(`/thumoiphdaunam/thumoiphdaunam/index.html?id=${sharedId}#${encoded}`);
          } else {
            setIframeLoadError('Không tìm thấy dữ liệu thư mời. Link có thể đã bị xóa hoặc chưa được lưu thành công.');
          }
        } catch (error) {
          console.error('[ThuMoiDauNam] Cannot load shared invitation:', error);
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (event.data && event.data.type === 'THU_MOI_SHARE') {
        ...
        if (config) {
          ...
          if (shortId) {
            const shortUrl = `${window.location.origin}?app=thu_moi_dau_nam&id=${shortId}`;
            ...
            const fallbackUrl = `${window.location.origin}/thumoiphdaunam/thumoiphdaunam/index.html?id=${shortId}#${encoded}`;
            iframeRef.current?.contentWindow?.postMessage({
              type: 'THU_MOI_SHORT_URL',
              url: fallbackUrl
            }, '*');
            ...
            console.error('[ThuMoiDauNam] Cannot save shared invitation:', error);
            const encoded = event.data.encoded;
            const fallbackUrl = encoded
              ? `${window.location.origin}/thumoiphdaunam/thumoiphdaunam/index.html#${encoded}`
              : '';
            iframeRef.current?.contentWindow?.postMessage({
  ...
  const handleEditInvitation = (item: any) => {
    ...
    setIframeLoadError('');
    setIframeSrc(`/thumoiphdaunam/thumoiphdaunam/index.html?edit=${item.shortId}#${encoded}`);
    setView('IFRAME');
  };
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-slate-800/80 backdrop-blur-sm border-b border-white/10 flex-shrink-0 z-10">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-white font-bold text-sm leading-tight">Thư Mời Họp Phụ Huynh Đầu Năm</h1>
              <p className="text-white/50 text-xs">{view === 'DASHBOARD' ? 'Bảng điều khiển giáo viên' : 'Tạo thư mời đầu năm tương tác'}</p>
            </div>
          </div>
        </div>
        {!sharedId && (
                ...
                {thuMoiList.map((item) => (
                  <div key={item.shortId} className="bg-slate-800 border border-slate-700 rounded-2xl p-6 hover:border-emerald-400/50 transition-colors group">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="bg-emerald-500/20 text-emerald-300 px-3 py-1 rounded-lg text-sm font-semibold flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => {
                            const url = `${window.location.origin}?app=thu_moi_dau_nam&id=${item.shortId}`;
        ...
        <div className="flex-1 relative">
          {iframeSrc ? (
            <iframe
              ref={iframeRef}
              src={iframeSrc}
              title="Thư Mời Họp Phụ Huynh Đầu Năm"
            ...
            <div className="flex h-full items-center justify-center bg-slate-950 p-6 text-white">
              <div className="max-w-md rounded-3xl border border-white/10 bg-white/10 p-6 text-center shadow-2xl">
                <Mail className="mx-auto mb-3 h-10 w-10 text-emerald-200" />
                ...
                <button
                  ...
                  className="mt-5 rounded-2xl bg-emerald-500 px-5 py-3 text-sm font-semibold text-white"
                >
                  Tải lại
                </button>
              </div>
