...
.card{position:relative;width:min(680px,96%);overflow:hidden;border-radius:${Math.max(22, playerTheme.radius)}px;padding:26px;background:${playerTheme.questionStyle === 'card' ? '#fff' : 'rgba(15,23,42,.92)'};border:1px solid ${playerTheme.questionStyle === 'card' ? 'rgba(15,23,42,.08)' : 'rgba(255,255,255,.22)'};box-shadow:0 28px 90px rgba(15,23,42,.38),0 0 0 1px ${playerTheme.primaryColor}22}.card:before{content:'';position:absolute;left:0;right:0;top:0;height:6px;background:linear-gradient(90deg,${playerTheme.primaryColor},${playerTheme.secondaryColor})}.qhead{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:2px 0 14px}.qtag,.qpoints{border-radius:999px;padding:6px 12px;font-size:11px;font-weight:950;text-transform:uppercase;letter-spacing:.05em}.qtag{background:linear-gradient(90deg,${playerTheme.primaryColor},${playerTheme.secondaryColor});color:#fff}.qpoints{background:#f1f5f9;color:#64748b}.qtitle{text-align:left;font-size:30px;line-height:1.14;font-weight:950;letter-spacing:-.02em;margin:0 0 8px}.qhint{margin:0 0 20px;color:${playerTheme.questionStyle === 'card' ? '#64748b' : 'rgba(255,255,255,.72)'};font-size:14px;font-weight:800}.option{width:100%;display:flex;align-items:center;gap:12px;border:1px solid #e2e8f0;border-radius:18px;background:rgba(255,255,255,.96);color:#1f2937;padding:13px 14px;margin:10px 0;text-align:left;font-weight:900;cursor:pointer;transition:.18s}.option:hover{transform:translateY(-1px);border-color:${playerTheme.primaryColor}66;box-shadow:0 12px 28px rgba(15,23,42,.10)}.option.selected{outline:0;border-color:transparent;box-shadow:0 0 0 3px ${playerTheme.primaryColor}55,0 14px 34px rgba(15,23,42,.14)}.olabel{display:grid;place-items:center;flex:0 0 36px;width:36px;height:36px;border-radius:12px;background:#f1f5f9;color:#64748b;font-weight:950}.option.selected .olabel{background:linear-gradient(135deg,${playerTheme.primaryColor},${playerTheme.secondaryColor});color:#fff}.otext{min-width:0;flex:1;line-height:1.35}.actions{display:flex;gap:12px;margin-top:18px}.actions button{flex:1;border:0;border-radius:16px;padding:14px 16px;color:#fff;font-weight:950;cursor:pointer;box-shadow:0 14px 30px rgba(15,23,42,.16)}
...
document.getElementById('answer').onclick=()=>{if(!current||selected===null)return;if(selected===current.correctOption){playCorrectSound();result.textContent='Ch\\u00ednh x\\u00e1c! +10 \\u0111i\\u1ec3m';answered.push(current.id);points+=10;score.textContent=points;scormSetScore(points,data.questions.length*10);setTimeout(()=>{overlay.classList.remove('show');if(answered.length===data.questions.length){firework();showBonus();scormComplete();showFinal()}else video.play()},900)}else{playIncorrectSound();result.textContent='Ch\\u01b0a \\u0111\\u00fang, em h\\u00e3y xem l\\u1ea1i \\u0111o\\u1ea1n video nh\\u00e9.'}};
