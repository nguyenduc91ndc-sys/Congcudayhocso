...
@media (max-width:700px){html,body{width:100%;max-width:100%;overflow-x:hidden;-webkit-text-size-adjust:100%}body.question-open{overflow:hidden!important}.app,.shell,.layout,.main,.player,.side{width:100%;max-width:100vw;min-width:0}.app{padding:0}.shell{border-left:0!important;border-right:0!important;border-radius:0!important;box-shadow:none!important}.topbar{height:auto;min-height:42px;padding:8px 12px;gap:8px}.topbar>span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.stats{gap:8px;font-size:12px;white-space:nowrap}.main{padding:0!important}.player{border-left:0!important;border-right:0!important;border-radius:0!important}.metaTop{padding:9px 12px}.brand{min-width:0;flex:1}.brand span:last-child{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.stage{width:100%;aspect-ratio:16/9;max-height:min(56.25vw,42vh)}video{display:block;width:100%;height:100%;object-fit:contain}.foot{padding:10px 12px;font-size:13px;line-height:1.35;overflow-wrap:anywhere}.controls{height:auto!important;display:grid!important;grid-template-columns:repeat(4,44px) minmax(46px,max-content) minmax(0,1fr);gap:8px;padding:10px 12px;overflow:hidden}.ctrl{width:44px;height:44px;border-radius:10px}.page{margin:0;text-align:center;font-size:20px;line-height:1;white-space:nowrap}.progress{min-width:0;width:100%;height:24px}.time,#full{display:none!important}.side{border-left:0!important;padding:24px 16px 28px!important;overflow:hidden!important}.school-logo{width:96px;height:96px;margin:18px auto 28px}.profile{min-width:0;align-items:center}.profile>div:last-child{min-width:0}.name,.role{overflow-wrap:anywhere}.name{font-size:22px;line-height:1.15}.role{font-size:16px;line-height:1.25}.info{min-height:48px}.tabs{width:100%;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:8px;margin:24px 0 12px}.tab{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding:14px 8px;font-size:16px}.search{width:100%;margin-bottom:20px}.overlay{position:fixed!important;inset:0!important;z-index:140!important;align-items:flex-start!important;justify-content:center!important;overflow-y:auto!important;-webkit-overflow-scrolling:touch;padding:14px 10px!important;background:rgba(2,6,23,.86)!important;backdrop-filter:blur(8px)}.overlay.show{display:flex!important}.card{width:min(100%,560px)!important;max-height:calc(100dvh - 28px);overflow-y:auto;-webkit-overflow-scrolling:touch;padding:16px;border-radius:20px;margin:auto 0}.qhead{gap:8px;align-items:flex-start}.qtag,.qpoints{font-size:10px;padding:6px 9px}.qtitle{text-align:left;font-size:clamp(18px,5vw,22px)!important;line-height:1.22!important;max-height:none;overflow:visible;overflow-wrap:anywhere;word-break:break-word}.qhint{font-size:13px;margin-bottom:14px}.option{align-items:flex-start;padding:11px 12px;margin:8px 0;border-radius:16px;overflow-wrap:anywhere;word-break:break-word}.option img{width:96px!important;flex-basis:96px!important}.olabel{flex:0 0 32px;width:32px;height:32px}.actions{display:grid;grid-template-columns:1fr;gap:8px;position:sticky;bottom:-16px;margin-left:-2px;margin-right:-2px;padding-top:8px;background:linear-gradient(180deg,transparent,rgba(15,23,42,.72) 20%,rgba(15,23,42,.92))}.actions button{padding:12px 14px}.final{align-items:flex-start;overflow-y:auto;padding:18px 12px}.final-body{padding:22px 16px}.final-actions{grid-template-columns:1fr}.gate{align-items:flex-start;overflow-y:auto;padding:18px 12px}.gate-card{padding:20px 16px;border-radius:22px}.avatars{grid-template-columns:repeat(3,1fr)}}
${exportLayoutCss}
${exportControlCss}
...
try{video.controls=false;video.setAttribute('playsinline','');video.setAttribute('webkit-playsinline','');video.setAttribute('x5-playsinline','');video.setAttribute('x5-video-player-type','h5');video.setAttribute('x5-video-player-fullscreen','false');video.setAttribute('x-webkit-airplay','deny');video.disablePictureInPicture=true}catch(e){}
...
async function sendGmailReportViaProxy(silent=false){const btn=silent?null:finalGmail,to=((data.theme&&data.theme.reportEmail)||'').trim()||(silent?'':(prompt('Nhap Gmail nhan bao cao:','')||'').trim());if(!to)return;const configuredReportUrl=((data.theme&&data.theme.reportApiUrl)||'').trim(),appsScriptUrl=/script\\.google\\.com|script\\.googleusercontent\\.com/i.test(configuredReportUrl)?configuredReportUrl:'',reportUrl=appsScriptUrl?'https://giaoviencn.io.vn/api/send-result-report':(configuredReportUrl||'https://giaoviencn.io.vn/api/send-result-report'),payload={to:to,subject:'Bao cao ket qua - '+data.title,text:buildReportText(),html:buildReportHtml(),learnerName:learnerName||'Hoc sinh',lessonTitle:data.title,appsScriptUrl:appsScriptUrl||undefined};try{if(btn){btn.disabled=true;btn.textContent='Dang gui'}const response=await fetch(reportUrl,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!response.ok){const error=await response.json().catch(()=>({}));throw new Error(error.error||'Khong gui duoc bao cao')}if(btn)btn.textContent='Da gui';if(!silent)alert('Da gui bao cao ve Gmail.')}catch(error){if(btn){btn.disabled=false;btn.textContent='Gui bao cao'}const message=error&&error.message?error.message:String(error||''),friendly=/failed to fetch|load failed|networkerror/i.test(message)?'Chua ket noi duoc API gui bao cao. Hay kiem tra link Apps Script hoac deploy lai API gui bao cao.':(message||'Khong gui duoc bao cao.');if(silent)console.warn('Khong gui duoc bao cao:',error);alert(friendly)}}
...
certBtn.onclick=()=>{certificateStage();sendGmailReportViaProxy(true)};
finalCert.onclick=()=>{certificateStage();sendGmailReportViaProxy(true)};
