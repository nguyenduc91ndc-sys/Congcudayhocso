...
const certificate=()=>{const learner=(prompt('Nhập tên học sinh để ghi thư khen:','Học sinh')||'Học sinh').trim()||'Học sinh',total=data.questions.length*10,date=new Date().toLocaleDateString('vi-VN'),w=window.open('','_blank');if(!w)return;w.document.write('<!doctype html><html lang="vi"><head><meta charset="utf-8"><title>Thư khen</title><style>body{margin:0;background:#f8fafc;font-family:Arial,sans-serif;color:#1f2937}.paper{width:900px;max-width:94%;margin:32px auto;padding:54px;border:12px solid #f59e0b;border-radius:28px;background:radial-gradient(circle at top,#fff7ed,#fff 42%);text-align:center;box-shadow:0 24px 80px rgba(15,23,42,.18)}h1{margin:0;color:#b45309;font-size:52px;text-transform:uppercase}.sub{font-size:20px;font-weight:700;color:#64748b}.name{margin:30px 0 18px;font-size:42px;font-weight:950;color:#7c3aed}.score{display:inline-block;margin:16px 0 26px;padding:12px 28px;border-radius:999px;background:#ecfdf5;color:#047857;font-size:24px;font-weight:950}.text{font-size:22px;line-height:1.55}.sign{display:flex;justify-content:space-between;margin-top:54px;font-weight:800}@media print{button{display:none}.paper{box-shadow:none;margin:0 auto}}</style></head><body><div class="paper"><h1>Thư khen</h1><p class="sub">Hoàn thành bài học tương tác</p><div class="name">'+learner.replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]))+'</div><p class="text">Đã hoàn thành bài học: <b>'+data.title+'</b><br>với tinh thần học tập tích cực.</p><div class="score">'+points+' / '+total+' điểm</div><div class="sign"><span>Ngày '+date+'</span><span>Giáo viên: ${authorName}</span></div><button onclick="window.print()" style="margin-top:32px;padding:12px 22px;border:0;border-radius:12px;background:#7c3aed;color:white;font-weight:900">In hoặc lưu PDF</button></div></body></html>');w.document.close();w.focus();setTimeout(()=>w.print(),400)};
