...
const InteractiveVideoModule: React.FC<InteractiveVideoModuleProps> = ({
    ...
    const createExportHtml = (videoFileName: string, scormVersion?: ScormVersion) => {
        ...
        const questionOverlayBackground = playerTheme.questionStyle === 'playful'
            ? 'rgba(2,6,23,.16)'
...
.card{position:relative;width:min(680px,96%);overflow:hidden;border-radius:${Math.max(22, playerTheme.radius)}px;padding:26px;background:${questionCardBackground};border:1px solid ${playerTheme.questionStyle === 'card' ? 'rgba(15,23,42,.08)' : 'rgba(255,255,255,.28)'};box-shadow:0 28px 90px rgba(15,23,42,.32),0 0 0 1px ${playerTheme.primaryColor}22}.card:before{content:'';position:absolute;left:0;right:0;top:0;height:${playerTheme.questionStyle === 'playful' ? '0' : '6px'};background:linear-gradient(90deg,${playerTheme.primaryColor},${playerTheme.secondaryColor})}.qhead{display:${playerTheme.questionStyle === 'playful' ? 'none' : 'flex'};align-items:center;justify-content:space-between;gap:12px;margin:2px 0 14px}.qtag,.qpoints{border-radius:999px;padding:6px 12px;font-size:11px;font-weight:950;text-transform:uppercase;letter-spacing:.05em}.qtag{background:linear-gradient(90deg,${playerTheme.primaryColor},${playerTheme.secondaryColor});color:#fff}.qpoints{background:#f1f5f9;color:#64748b}.qtitle{text-align:${playerTheme.questionStyle === 'playful' ? 'center' : 'left'};font-size:${playerTheme.questionStyle === 'playful' ? '24px' : '30px'};line-height:1.14;font-weight:950;letter-spacing:-.02em;margin:0 0 ${playerTheme.questionStyle === 'playful' ? '18px' : '8px'}}.qhint{display:${playerTheme.questionStyle === 'playful' ? 'none' : 'block'};margin:0 0 20px;color:${playerTheme.questionStyle === 'card' ? '#64748b' : 'rgba(255,255,255,.72)'};font-size:14px;font-weight:800}.option{width:100%;display:flex;align-items:center;gap:12px;border:1px solid #e2e8f0;border-radius:${playerTheme.questionStyle === 'playful' ? '999px' : '18px'};background:rgba(255,255,255,.96);color:#1f2937;padding:${playerTheme.questionStyle === 'playful' ? '12px 16px' : '13px 14px'};margin:10px 0;text-align:left;font-weight:900;cursor:pointer;transition:.18s}.option:hover{transform:translateY(-1px);border-color:${playerTheme.primaryColor}66;box-shadow:0 12px 28px rgba(15,23,42,.10)}.option.selected{outline:0;border-color:transparent;box-shadow:0 0 0 3px ${playerTheme.primaryColor}55,0 14px 34px rgba(15,23,42,.14)}.option.selected:after{content:'✓';display:grid;place-items:center;flex:0 0 20px;width:20px;height:20px;border-radius:999px;background:#10b981;color:#fff;font-size:13px;font-weight:950}.olabel{display:${playerTheme.questionStyle === 'playful' ? 'none' : 'grid'};place-items:center;flex:0 0 36px;width:36px;height:36px;border-radius:12px;background:#f1f5f9;color:#64748b;font-weight:950}.option.selected .olabel{background:linear-gradient(135deg,${playerTheme.primaryColor},${playerTheme.secondaryColor});color:#fff}.otext{min-width:0;flex:1;line-height:1.35}.actions{display:flex;gap:12px;margin-top:18px}.actions button{flex:1;border:0;border-radius:${playerTheme.questionStyle === 'playful' ? '999px' : '16px'};padding:14px 16px;color:#fff;font-weight:950;cursor:pointer;box-shadow:0 14px 30px rgba(15,23,42,.16)}
