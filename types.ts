...
export type ViewState = 'LOGIN' | 'DASHBOARD' | 'CREATE_EDIT' | 'PLAYER' | 'ADMIN' | 'GEOMETRY_3D' | 'BEE_GAME' | 'BEE_GAME_EDITABLE' | 'BACTERIA_GAME' | 'VONG_QUAY' | 'LUCKY_WHEEL' | 'KING_GAME' | 'KING_GAME_LOP_HOC_COMPACT' | 'STAR_WHEEL' | 'VIDEO_STORE' | 'INTERACTIVE_VIDEO' | 'AI_COURSE_STORE' | 'AI_COURSE_ADMIN' | 'CANVA_BASICS' | 'NEW_YEAR' | 'COMMUNITY_RESOURCES' | 'DEN_HUNG_3D' | 'HEART_SYSTEM_3D' | 'VIETNAM_MAP' | 'CHUC_TET' | 'PUZZLE_GAME' | 'TREASURE_HUNT' | 'VIRTUAL_EXPERIMENT' | 'CLOCK_EXPERIMENT' | 'BANG_CUU_CHUONG' | 'GAME_TUONG_TAC' | 'YOGURT_EXPERIMENT' | 'KIEM_TRA_DAO_VAN' | 'ABOUT' | 'PRIVACY' | 'TERMS' | 'CONTACT' | 'PHONG_TRANH_3D' | 'SANG_KIEN_KN' | 'EARTH_SEASONS' | 'THAT_LUONG_3D' | 'NHAN_XET_TT27' | 'NHAY_BAO_BO' | 'SOLAR_SYSTEM' | 'KEO_CO_TRI_TUE' | 'GAME_TUY_CHINH' | 'DINH_DOC_LAP_3D' | 'KY_YEU_CUOI_NAM' | 'THIEP_MOI_ONLINE';
